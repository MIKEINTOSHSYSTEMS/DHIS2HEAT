...
format_analytics_data <- function(analytics_data, indicators, org_units, population_data, indicator_map, dimension = "Region", settings_abbr = NULL, custom_scales = NULL) {
    ...
    n_headers <- nrow(headers) # Number of rows in the headers data.frame
    ...
    col_names <- headers$name # Use the 'name' column from headers
    ...
    if ("saIPeABoPMH" %in% colnames(rows)) {
        ...
        rows$saIPeABoPMH <- NULL # Remove the ID column
        ...
        rows$hxLghWPCAXE <- NULL # Remove the ID column
    } else {
        rows$dimension <- dimension

        # Set subgroup based on dimension
        if (dimension == "Region") {
            rows$subgroup <- org_units$displayName[match(rows$ou, org_units$id)]
        } else if (dimension == "Zone") {
            # Fetch zone metadata if not already available
            if (!exists("zones_metadata")) {
                zones_metadata <- get_dhis2_data("/api/organisationUnits?fields=id,displayName&level=3&paging=false")$organisationUnits
            }
            rows$subgroup <- zones_metadata$displayName[match(rows$ou, zones_metadata$id)]
        } else if (dimension == "Woreda") {
            # Fetch woreda metadata if not already available
            if (!exists("woredas_metadata")) {
            ...
            rows$subgroup <- woredas_metadata$displayName[match(rows$ou, woredas_metadata$id)]
        } else {
            stop("Unsupported dimension:", dimension)
        }
    }

    # Debug: Print columns in 'rows' and 'population_data'
    cat("Columns in 'rows':", paste(colnames(rows), collapse = ", "), "\n")
    cat(
        "Columns in 'population_data':",
        paste(colnames(population_data), collapse = ", "), "\n"
    )

    # Merge with population data
    rows <- merge(rows, population_data, by.x = c("ou", "pe"), by.y = c("Organisation_Unit_ID", "Period"), all.x = TRUE)
